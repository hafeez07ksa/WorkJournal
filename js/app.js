/* =============================================
   ...
   =============================================
   VIEW MODE  → public, boss can read all logs
   WRITE MODE → password protected, only you

   Default password: reporter2024
   Change it on line 15 below.
   ============================================= */
...
let _allReports = { daily: [], weekly: [] }; // in-memory cache
...
async function sheetGet(type) {
  ...
  // Apps Script returns JSONP: callback({...}) — strip the wrapper
  text = text.replace(/^[a-zA-Z_][a-zA-Z0-9_]*\s*\(/, "").replace(/\);\s*$/, "");
...
async function sheetSave(payload) {
  ...
  } catch(e) { console.warn("sheetSave error:", e); }
  return { success: true };
}
...
async function sheetDelete(type, id) {
  ...
  } catch(e) { console.warn("sheetDelete error:", e); }
  return { success: true };
}
...
async function loadAllReports() {
  try {
    const [daily, weekly] = await Promise.all([sheetGet("daily"), sheetGet("weekly")]);
...
function showLoading(on) {
  ["daily-logs-list", "weekly-logs-list"].forEach(id => {
    if (on) document.getElementById(id).innerHTML = '<p class="empty-state loading-state"><i class="fa fa-circle-notch fa-spin"></i> Loading reports…</p>';
  });
}
...
async function saveDaily() {
  ...
  try {
    ...
    if (idx > -1) _allReports.daily[idx] = entry;
    else _allReports.daily.unshift(entry);
...
function renderDailyLogs() {
  ...
  list.innerHTML = reports.map(r => dailyCard(r)).join("");
}

function dailyCard(r) {
  const preview  = stripHtml(r.content).slice(0, 160);
  const time     = r.savedAt ? new Date(r.savedAt).toLocaleTimeString("en-US",
  ...
  return `
  <div class="log-card clickable" onclick="openReadModal('daily','${r.id}')" id="card-d-${r.id}">
...
async function saveWeekly() {
  ...
  try {
    ...
    if (idx > -1) _allReports.weekly[idx] = entry;
    else _allReports.weekly.unshift(entry);
...
function renderWeeklyLogs() {
  ...
  if (!reports.length) {
    list.innerHTML = '<p class="empty-state">No weekly reports yet.</p>';
    return;
  }
  list.innerHTML = reports.map(r => weeklyCard(r)).join("");
}
...
function weeklyCard(r) {
  ...
  return `
  <div class="log-card clickable" onclick="openReadModal('weekly','${r.id}')">
    <div class="log-card-header">
      <div>
...
function openReadModal(type, id) {
  const reports = type === "daily" ? _allReports.daily : _allReports.weekly;
...
async function saveEdit() {
  ...
  const reports = _editType === "daily" ? _allReports.daily : _allReports.weekly;
...
/* =============================================
   MOBILE BACK BUTTON — close modals on back
   ============================================= */
...
function closeAnyOpenModal() {
  const readOpen = !document.getElementById("read-modal").classList.contains("hidden");
  const editOpen = !document.getElementById("edit-modal").classList.contains("hidden");
  const unlockOpen = !document.getElementById("unlock-modal").classList.contains("hidden");
  if (readOpen)   { closeReadModal();   return true; }
  if (editOpen)   { closeModal();       return true; }
  if (unlockOpen) { closeUnlockModal(); return true; }
  return false;
}

window.addEventListener("popstate", function(e) {
  if (closeAnyOpenModal()) {
    // Modal was closed — stay on page, don't navigate back
    // Push state again only if another modal is still open
    const anyStillOpen = !document.getElementById("read-modal").classList.contains("hidden")
      || !document.getElementById("edit-modal").classList.contains("hidden")
      || !document.getElementById("unlock-modal").classList.contains("hidden");
    if (anyStillOpen) history.pushState({ modal: true }, "");
  }
});
...
async function deleteReport(type, id) {
  ...
  try {
    ...
    if (type === "daily") _allReports.daily = _allReports.daily.filter(r => r.id !== id);
    else _allReports.weekly = _allReports.weekly.filter(r => r.id !== id);
